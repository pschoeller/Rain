...
public class Screen {
	...
	public void renderSprite(int xp, int yp, Sprite sprite, boolean fixed, double angle){
		if(fixed){
			xp -= xOffset;
			yp -= yOffset;
		}
	...
	public void renderProjectile(int xp, int yp, Sprite sprite, boolean fixed, double angle){
		if(fixed){
			xp -= xOffset;
			yp -= yOffset;
		}
		...
		for(int y=0; y<spriteHeight; y++){
			...
			for(int x=0; x<spriteWidth; x++){
				...
				if(xa < -spriteWidth || xa >= width || ya < 0 || ya >= height) continue;
				if(xa < 0) xa = 0;
				int[] rp = rotate(sprite.getPixels(), spriteWidth, spriteHeight, angle);
				int color = rp[xs + ys * spriteWidth];
				if(color != ALPHA_COL) pixels[xa + ya * width] = color;
			}
		}
	}
	...
	private int[] rotate(int[] pixels, int width, int height, double angle){
		...
		for(int y=0; y<height; y++){
			...
			for(int x=0; x<width; x++){
				...
				int yy = (int)y1;
				int col=0;
				if(xx < 0 || xx >= width || yy < 0 || yy >= height) col = ALPHA_COL;
